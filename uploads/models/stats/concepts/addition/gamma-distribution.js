...
let histogram_max =
    sim_alpha * sim_theta * sim_n_sum +
    HISTOGRAM_INTERVAL * Math.sqrt(sim_alpha * sim_n_sum) * sim_theta;
...
function generate_multiple_samples(n_samples, alpha, theta, n_sum) {
    return Array(n_samples)
        .fill(null)
        .map(() => generate_single_sample(alpha, theta, n_sum));
}

function generate_single_sample(alpha, theta, n_sum) {
    return Array(n_sum)
        .fill(null)
        .map(() => jStat.gamma.sample(alpha, theta))
        .reduce((a, c) => a + c);
}
...
generate_btn.addEventListener("click", () => {
    ...
    theory_alpha = sim_alpha * sim_n_sum;
