...
let sim_alpha = 2;
let theory_alpha = sim_alpha;
let sim_theta = [0.5, 1, 1.5];
let theory_theta = sim_theta.reduce((a, c) => a + c, 0);
let n_runs = 0;
...
function generate_multiple_samples(n_samples, alpha, theta) {
    return Array(n_samples)
        ...
        .map(
            () =>
                jStat.gamma.sample(alpha, theta[0]) +
                jStat.gamma.sample(alpha, theta[1]) +
                jStat.gamma.sample(alpha, theta[2])
        );
}
...
generate_btn.addEventListener("click", () => {
    ...
    sim_theta = [
        my_parse_float(document.getElementById("theta_1").value),
        my_parse_float(document.getElementById("theta_2").value),
        my_parse_float(document.getElementById("theta_3").value),
    ];
    n_runs = 0;

    const theory_mean = sim_alpha * sim_theta.reduce((a, c) => a + c, 0);
    const theory_var = sim_alpha * sim_theta.reduce((a, c) => a + c * c, 0);

    theory_alpha = (theory_mean * theory_mean) / theory_var;
    document.getElementById("theory_alpha").value = theory_alpha.toFixed(2);
    theory_theta = theory_var / theory_mean;
    document.getElementById("theory_theta").value = theory_theta.toFixed(2);
