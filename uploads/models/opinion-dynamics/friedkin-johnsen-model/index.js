...
let initial_state = current_state.slice();
...
function initialize() {
    ...
    stubbornness = my_parse_float(
        document.getElementById("stubbornness").value
    );
    ...
    initial_state = current_state.slice();
...
function simulate_step() {
    ...
    for (let i = 0; i < N_AGENTS; i += 1) {
        current_state[i] = stubbornness * initial_state[i];
        for (let j = 0; j < N_AGENTS; j += 1) {
            current_state[i] +=
                (1 - stubbornness) * interaction_matrix[i][j] * old_state[j];
        }
    }
}
