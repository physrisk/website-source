...
function ai_pick_priorities() {
    let pick = Math.floor(Math.random() * AI_PRIORITIES.length);
    return AI_PRIORITIES[pick].slice();
}

function ai_pick_castle(weights) {
...
function ai_strategy() {
    let priority_castles = ai_pick_priorities();
    ...
    while (remaining_troops > 0) {
        let which_priority = ai_pick_castle(priority_weights);
