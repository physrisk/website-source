...
function get_theory(freqs, min_rate, max_rate) {
    return freqs.map((f) => {
        ...
        let norm_term = 1 / Math.log(max_rate / min_rate);
        return (norm_term * diff_term) / (2 * half_omega);
    });
}
...
function get_lorentzian(freqs, generation_rate, recombination_rate) {
    ...
    let bar_nu = 1 / (1 / generation_rate + 1 / recombination_rate);
    return freqs.map((f) => {
        let omega = 2 * Math.PI * f;
        return (4 * bar_nu) / (rate_sum_sq + Math.pow(omega, 2));
    });
}
...
function run(min_rate = 1e-6, max_rate = 1) {
    ...
    for (let step = 0; step < STEPS; step += 1) {
        generation_rate =
            Math.pow(max_rate / min_rate, rng.random()) * min_rate;
