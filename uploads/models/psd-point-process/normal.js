...
function generate_time(mean, std_dev) {
    let tau = rng.normal(mean, std_dev);
    while (tau < 0) {
        tau = rng.normal(mean, std_dev);
    }
    return tau;
}

function generate_series(mean = 1, std_dev = 0.1) {
    let series = Array(OBSERVATION_STEPS).fill(0);
    let n_events = 0;
    let tau = generate_time(mean, std_dev);
    let event_time = tau;
    while (event_time < OBSERVATION_TIME) {
        series[Math.floor(event_time / PERIOD)] += 1;
        n_events += 1;
        tau = generate_time(mean, std_dev);
...
function run(mean_time = 1e-4, std_time = 1e-5) {
    let series = generate_series(mean_time, std_time);
...
generate_btn.addEventListener("click", () => {
    let mean_time = Math.pow(
        10,
        my_parse_float(document.getElementById("mean_time").value)
    );
    let std_time = Math.pow(
        10,
        my_parse_float(document.getElementById("std_time").value)
    );
    run(mean_time, std_time);
});
